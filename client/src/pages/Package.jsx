...
const Package = () => {
  ...
  const giveRating = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/rating/give-rating`, {
        method: "POST",
        headers: {
  ...
  const getRatings = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/rating/get-ratings/${params.id}/4`);
      const data = await res.json();
      if (data) {
  ...
  const checkRatingGiven = async () => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/rating/rating-given/${currentUser?._id}/${params?.id}`
      );
      const data = await res.json();
